...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in actions:
            value = max(self.getQValue(state, action), value)
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in actions:
            if value == self.getQValue(state, action):
                this.append(action)
        max_arg = len(this)
        index = int(random.random()/max_arg)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for index in featureVectors.keys():
            self.weights[index] += self.alpha * difference * featureVectors[index]
